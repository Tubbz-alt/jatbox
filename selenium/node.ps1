...
if($RUNNING)
{
	...
	cd $DIR
	java -jar selenium-server-standalone-2.47.1.jar -role node -hub http://${HUB_IP}:${HUB_PORT}/grid/register '-browser browserName=internet explorer,maxInstances=5' '-Dwebdriver.chrome.driver=chromedriver.exe' '-Dwebdriver.ie.driver=IEDriverServer.exe'
